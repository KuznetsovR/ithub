...
export default class Portal extends EventEmitter {
  ...
  setPortal() {
    this.portal = {
      geometry: new THREE.CircleGeometry(this.size, 6),
    ...
    this.time.on('tick', () => {
      if (this.timer.currentPortal === null || this.timer.currentPortal === this.name) {
        if (this.elapsedTimeInPortal > 100) {
          ...
          console.log(this.elapsedTimeInPortal, this.timer);
  ...
  observeSkate() {
    this.time.on('tick', () => {
      const skateIsIn =
        Math.abs(this.mainHero.position.x - this.position.x) < Math.abs(this.size / 2) * 1.5 &&
        Math.abs(this.mainHero.position.y - this.position.y) < Math.abs(this.size / 2) * 1.5;
